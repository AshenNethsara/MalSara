...
Sara.addCommand({pattern: 'print ?(.*)', fromMe: true, desc: dd}, (async (message, match) => {    
...
Sara.addCommand({pattern: 'bashmedia ?(.*)', fromMe: wk_q, desc: bdesc, usage: 'video.mp4 && media/gif/pic.mp4'}, (async (message, match) => {    
    var id = message.jid
    try {
        if (match[1].includes('jpg') || match[1].includes('tiff') || match[1].includes('raw') || match[1].includes('dng') || match[1].includes('png') || match[1].includes('jpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/MalSara/${match[1]}`), MessageType.image, {caption: 'Made by MalSara' })
        }
        else if (match[1].includes('mp4') || match[1].includes('avi') || match[1].includes('webm') || match[1].includes('mkv') || match[1].includes('mpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/MalSara/${match[1]}`), MessageType.video, {caption: 'Made by MalSara' });
        }
            ...
            await message.client.sendMessage(id,fs.readFileSync(`/root/MalSara/${match[1]}`), MessageType.audio);
        }
        else {
...
Sara.addCommand({pattern: 'addserver$', fromMe: wk_ad, desc: addsdesc}, (async (message, match) => {    
    if (message.reply_message.image) {
        ...
        exec('mv ' + location + ' /root/MalSara/server-image.' + fin)
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/MalSara/server-video.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/MalSara/server-audio.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
...
Sara.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
...
Sara.addCommand({pattern: 'term ?(.*)', fromMe: true, desc: Lang.TERM_DESC}, (async (message, match) => {    
...
Sara.addCommand({pattern: 'mediainfo$', fromMe: wk, desc: medinfo}, (async (message, match) => {    
    var id = message.jid
    if (message.reply_message.video) {
        ...
        exec('mv ' + location + ' /root/MalSara/vid.mp4')
        exec('ffprobe -hide_banner -loglevel fatal -show_error -show_format -show_streams -show_programs -show_chapters -show_private_data -print_format json /root/MalSara/vid.mp4', async (err, st, stderr) => {
            if (err) {
...
Sara.addCommand({pattern: 'pmsend ?(.*)', fromMe: true, desc: pmmm }, (async (message, match) => {
...
Sara.addCommand({pattern: 'pmttssend ?(.*)', fromMe: true, desc: psmm}, (async (message, match) => {
    if (!message.reply_message) return await message.client.sendMessage(message.jid,NLang.NEED_REPLY, MessageType.text);
    if (message.reply_message && match[1] == '') return await message.client.sendMessage(message.jid, NLang.NEED_WORDS, MessageType.text);
    let 
